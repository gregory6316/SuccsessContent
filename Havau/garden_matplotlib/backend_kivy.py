...
class RendererKivy(RendererBase):
    """The kivy renderer handles drawing/rendering operations.
       
    A RendererKivy should be initialized with a FigureCanvasKivy widget. On initialization
    ...
    def contains(self, widget, x, y):
        """Return whether or not a point is inside the widget.

        The value of the point is defined in x, y as kivy coordinates.
        """
        left = widget.x
        bottom = widget.y
    ...
    def handle_clip_rectangle(self, gc, x, y):
        """Check whether the point (x,y) collides with any already existent stencil.

        If so it returns the index position of the
    ...
    def collides_with_existent_stencil(self, x, y):
        """Check all the clipareas and returns the index of the clip area that contains this point.

        The point x, y is given in kivy coordinates.
        """
        idx = -1
    ...
    def get_path_instructions(self, gc, polygons, closed=False, rgbFace=None):
        """With a graphics context and a set of polygons it returns a list of InstructionGroups required to render the path."""
        instructions_list = []
        points_line = []
    ...
    def draw_mathtext(self, gc, x, y, s, prop, angle):
        ...
        The position x,y is given in Kivy coordinates.
        """
    ...
    def weight_as_number(self, weight):
        """Replace the deprecated matplotlib function of the same name."""
        # Return if number
...
class NavigationToolbar2Kivy(NavigationToolbar2):
    ...
    def show_save(self):
        """Display a popup widget to perform a save operation."""
...
class GraphicsContextKivy(GraphicsContextBase, object):
    ...
    def _get_style_dict(self, rgbFace):
        """Return the style string. style is generated from the GraphicsContext and rgbFace."""
        attrib = {}
